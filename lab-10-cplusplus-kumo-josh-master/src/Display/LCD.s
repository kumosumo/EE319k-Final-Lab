...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
	PUSH{R4, R5}
loop	LDR R4, =SSI0_SR_R	;first loop
	LDR R5, [R4]
	AND R5, R5, #0x10
	CMP R5, #0x10
	BEQ loop
	...
	LDR R5, = DC_COMMAND
	STR R5, [R4]
	LDR R4, =SSI0_DR_R		;Store command in SSI0_DR_R	
	STRB R0, [R4]
loop2	LDR R4, =SSI0_SR_R	;second loop
	LDR R5, [R4]
	AND R5, R5, #0x10
	CMP R5, #0x10
	BEQ loop2
...
loopwr	LDR R4, =SSI0_SR_R	;loop
	...
	STR R5, [R4]
	AND R5, R5, #0
	LDR R4, =SSI0_DR_R		;Store data to DR
	STRB R0, [R4]
	POP {R4, R5}
    BX  LR                          ;   return
